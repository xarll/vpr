...
### 5. Монтирование файловой системы на зашифрованом разделе
...
Предлагаю для наглядости создать два ключа и удалить их.
...
Проведём поиск по цифре **3** (слот, который занимал ключ) в **luksDump** для того, что бы снова не переписывать файл. Смотрим результат.
```
...
Для демонстрации корректной работы шифрованого раздела предлагаю скопировать файл содержащий **luksDump** (информацию о ключах в слотах) в наш раздел.
```
...
Чтобы ОС сама научилась подключать и монтировать нужные криптовынные устройства во время загрузки, а затем корректно отключать их во время останова системы, необходимо добавить следующее.
