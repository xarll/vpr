...
`^(?:+7|8)(?: ?\d3 ?|\d{3}(?: ?\d{2}){2}|(?: ?\d{2}){3})$`

- `^` - начало строки
- `(?:+7|8)` - начало номера - либо символы "+7", либо "8", вне группы, чтобы не захватывать их в отдельный результат
- `(?: ?\d3 ?|\d{3}(?: ?\d{2}){2}|(?: ?\d{2}){3})` - тело номера, также вне группы, чтобы не захватывать его в отдельный результат. Состоит из трех возможных шаблонов:
    - `?\d3` - пробел или несколько пробелов, затем "(" тройка цифр ")" и пробел или несколько пробелов
    - `\d{3}(?: ?\d{2}){2}` - тройка цифр, затем 0 или 1 пробела и 2 цифры, повторяем 2 раза (т.е. еще 2 пары цифр после первой тройки)
    - `(?: ?\d{2}){3}` - 0 или 1 пробел, затем две цифры, повторяем 3 раза (т.е. еще 3 пары цифр после первой тройки)
    - `$` - конец строки
