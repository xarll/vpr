...
- Необходимо согласиться на установку драйвера когда появиться соответсвующее диалоговое окно
...
Нам потребуется:
- Для оценки <<отлично>> модель MUA -> MTA -> MTA: 2 сервера и 1 клиент => всего 3 машины. (в разработке)
- Для <<оценки>> модель MUA -> MTA: 1 сервер 1 клиент => всего 2 машины. (Сделано)

Где:
- `MUA` = mail user agent - в нашем случае клиент
- `MTA` = mail transfer agent - в нашем случае почтовый сервер

Все домыслы и догадки по поводу моделей не подтверждены, кроме MUA -> MTA -> MTA (в разработке)<br>
Модель выберите сами. Если есть полезная информация, прошу в [VK](https://vk.com/jkearnsl)
...
ВМ создана. Запускаем её, устанавливаем ОС (процесс установки можно глянуть в инструкции 2й лабы).<br>
После установки ОС нужно послать сингнал завершения работы и клонировать машину столько раз, сколько потребуется(зависит от того, какую модель вы выбрали):
- ![settings](https://i.imgur.com/miR7JtM.png)
- ![settings](https://i.imgur.com/UDaONlr.png)
- ![settings](https://i.imgur.com/IZFBw6f.png)
Повторяем процедуру ещё 1 раз и получаем следующую картину(в моем случае я создавал 3 машины, для модели MUA -> MTA -> MUA, которая, как выяснилось, ненужна):
...
Для серверной ВМ (в моем случае "mail-server") следует установить следующие компоненты: sendmail, ...<br>
...
Для клиентских ВМ (в моем случае "mail-client" и "mail-client2") следует установить следующие компоненты: mutt, ...<br>
Чтобы это сделать, пропишите следующее в консоль:
```sh
sudo su
```
...
Но вы столкнетесь с проблемой того, что у Вас не будет доступа к интернету<br>
потому что мы выбрали виртуальный хост адаптера (более надежный, чем NAT, но последний предоставляет доступ к интернету)<br>
...
Чтобы нам не страдать, советую собрать ip всех машин в столбик,<br>
а так же настроить статические ip на всех машинах.
...
Следующую процедуру следует повторить для каждой ВМ,<br>
...
Обратите внимание: стоит соблюдать отступы(пробелы)<br>
совет: смотрите под какой буквой какая строка начинается.

Далее жмем `Ctrl+O` , далее `Enter` , далее `Ctrl+X`<br>
...
Готово! Если нет ошибок, то всё получилось, <br>
...
Соберем информацию о машинах в столбик (в моем случае я создавал 3 машины, для модели MUA -> MTA -> MUA, которая, как выяснилось, ненужна)
...
В файле /etc/mail/access устанавливаются права доступа<br>
Дадим разрешение на отправку почты от наших клиентов(ВМ "mail-client" и "mail-client2" 
(PS: в моем случае я создавал 3 машины, для модели MUA -> MTA -> MUA, которая, как выяснилось, ненужна => вам нужен всего 1 клиент и кол-во серверов на Ваш вкус)), прописав их ip адреса в столбик через любой редактор (например nano/vim) или просто введя в консоль:
...
Объявляем имя домена, для которого sendmail адреса почтовых ящиков будет считать<br>
локальными. Эту процедура нужна для того, чтобы занимать "красивые" адреса в локальной сети, типа<br>
"example@nikita.com"; Введенные доменные имена будут считаться локальными.<br>
...
Так как в прошлом пункте мы задали в таблицу доменов красивое имя, <br>
то теперь мы должны задать для этого имени адрес сервера, на который будет оно ссылаться!<br>
...
Где:
- Первый столбик - адрес вашей серверной машины (mail-server в моем случ);
- Второй - красивое локальное доменное имя, что я задал выше;
- Третий - сетевое имя ВМ.
...
`Ctrl+O` , далее `Enter` , далее `Ctrl+X`. Готово.<br>
...
Клиентские ВМ настраиваются следующим образом.<br>
Я буду провидить действия от имени машины `mail-client`.<br>
Приступим!
...
## Модель: MUA -> MTA
Настроим для модели MUA -> MTA. Нам понадобится `mail-server` и `mail-client`; <br>
Я буду провидить действия от имени машины `mail-client`.<br>
Приступим!
...
Проверка, что письмо дошло до ВМ `mail-server`:<br>
...
Для <<оценки>> этого вроде достаточно.

## Модель  MUA -> MTA -> MTA (продолжение ниже)
Если вы хотите получить оценку ОТЛ и быть уверенным, что лабу вообще зачтут, то советую выбрать эту модель
Передача сообщения от MUA (клиента) через MTA1 в MTA2


## Модель  MUA -> MTA -> MUA (КАК ВЫЯСНИЛОСЬ, НЕ НУЖНА!!! => этот пункт излишен)
Теперь мы умеем отправлять сообщения, но для работы модели  MUA -> MTA -> MUA нам нужно<br>
отправить сообщение конечному пользователю, а мы пока отправили только в MTA<br>
...
ОШИБКА, т.к нужен MDA (служба доставки писем) а в лабе она не усматривается => если кому интересно, то устанавливайте какой-нибудь POP3 или etc


# Модель MUA - MTA - MTA (та самая завтеная) - Инструкция от Горшочка

## Список ВМ

Нам нужно три ВМ

1. CLIENT - клиент машина, в ней всё так же как бояр писал выше. (утилита mutt и т.д.)
2. ONE - первый почтовый сервер MTA. Так же как и у бояра (sendmail)
3. TWO - второй почтовый сервер MTA. Аналогично

### Всё выполняем через `sudo su`!!!

## Кратко что вообще происходит

Немного теории. 

У нас есть клиент и два почтовых сервера. Проведём аналогию с реальным миром. One - сервер gmail.com, а Two - сервер yandex.ru.

Мы (Петя) зашли с нашего компа (клиента) и залогинились в почтовый ящик гугла (petya@gmail.com). Мы решили отправить письмо другу по имени Вася (vasya@yamdex.ru), но у него почтовый ящик расположен на серверах яндекса, в то время как наш на серверах гугла. Что же делать? Ничего. Сервер гугла сам перенаправит письмо от Пети для Васи на сервер Яндекса и все остануться довольными.

Так вот. Наша задача как раз настроить тот самый сервер гугла (one) и сервер яндекса (two).

## Настройки сети для ВМ

Параметры сети: `Виртуальный адаптер хоста`

Если вы прописывали статический ip до изменения параметров сети, то выполните `netplan apply` чтобы настройки применились заново

Необходимо каждой ВМ задать статически ip адреса согласно инструкции бояра (netplan)

И давайте выпишем их в табличку

| №   | Имя          | IP             | Домен  |
|-----|--------------|----------------|--------|
| 1   | Client (mua) | 192.168.56.105 | -      |
| 2   | One (mta)    | 192.168.56.106 | one.ru |
| 3   | Two (mta)    | 192.168.56.107 | two.ru |

## Кастомные доменные имена

Выполняется для обоих серверов (one и two)

```bash
nano /etc/hosts
```
И согласно нашей табличке пропишем ip и домены

![hosts](https://i.imgur.com/YNbW0of.png)

По умолчанию тут настройки совсем другие

Алгоритм изменения:

- Первую запись `127.0.0.1 localhost` мы не трогаем
- Всё что идёт после синего текста тоже не трогаем
- Вторая запись состоит из трёх слов. Запоминаем последнее. В моём случае это `white`
- Стираем вторую запись 
- Теперь дописываем наши ip и домены по таблице так, чтобы получилось как на скрине выше
- Добавляем наше слово (`white`) в конец к той записи, в которой прописан ip текущей машины (тут машина one, дописываем после записи для машины one)

Готово. Сохраняем изменения и выходим из nano.

## Запретим использование стандартного dns
...
И стираем слово `dns` в этом файле. Сохраняем.

Правильный вид:

![switch service](https://i.imgur.com/7ad4KeH.png)

## Правим файл access

Для обоих ВМ, в каждом приписывается два остальные ip из таблицы

Файл `access` можно править по инструкции бояра.

Итоговый вид моего файла для ONE:
![one access file](https://i.imgur.com/U6JZjjm.png)

Не забудьте собрать файл!


## Правим domaintable

Для обоих вм. В файле прописывается домен, который принадлежит этой вм
...
Для обоих вм, но на каждой машине прописываем местами свой ip

```bash
nano /etc/mail/sendmail.cf
```

Для дальнейших правок необходимо использовать поиск в nano. Гайд по нему почитайте в интернете. 

Моменты которые необходимо поправить:

1. Те же строки, что и в инструкции от бояра
2. Бояр пропустил ещё строчку, но мы то её не пропустим :)
![sf](https://i.imgur.com/CbINp3P.png)

### Не забываем перезагрузить демон почтового сервера `systemctl restart sendmail`

## Почта, просмотр и клиент

- Настройка клиента идентична, что и в инструкции от бояра
- Отправляем почту на root@two.ru
- Просмотр логов на второй машине `cat /var/log/mail.log`
- Почтовый ящик нам надо смотреть на машине 3

# Вывод
Какую выбрать модель - решать вам. Все модели составлены по домыслам студентов.<br>
Лаба сложная и моментами непонятная. Я не гарантирую, что модель MUA -> MTA вообще прокатит, как и остальные.<br>
Если у Вас есть полезная информация, милости прошу в [VK](https://vk.com/jkearnsl).
