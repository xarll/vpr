...
```sh
ssh white@192.168.0.112
```

- `white` - это имя пользователя линукс
- `192.168.0.112` - ip от ВМ 3

1. У вас спросят про довереность отпечатка ключа. Пишем ему в ответ слово `yes` и жмём enter
2. Прописывайте пароль от пользователя
3. Повышаем права `sudo su`
...
5. Теперь вернёмся к ВМ 1 и выйдем из ssh сессии
6. Пишем `exit`. Сейчас мы ещё в сессии, но вышли из суперпользователя
7. Снова пишем `exit`. Теперь мы вышли из ssh сессии
